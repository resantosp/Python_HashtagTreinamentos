...
for i, lista in enumerate(estoque):
    #saber se dentro de cada lista tem alguém abaixo do mínimo
    for item in lista:
        if item < minimo:
            if fabricas[i] in fabricas_minimo:
                pass
            else:
                fabricas_minimo.append(fabricas[i])
print(fabricas_minimo)
            ##print('A fábrica {} apresenta um item com apenas {} em estoque.'.format(fabricas[i], item))